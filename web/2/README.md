...
https://quiz.ais3.org:23525/


## Write-up

進去後是一個登入網頁，直接點Get the source。
![](./login.png)
...
?>
```
大概的內容是將前頁輸入的帳號密碼跟這裡的帳號密碼比對，但是這裡的密碼經果md5 hash過，因此不知道原本密碼是什麼。雖然md5的演算法是不可逆的，但是網路上有許多用彩虹表直接對應的方式。

![](./found.jpeg)

最後可以得到 `sena` 的密碼是 `14033368894`，登入後得到Flag
